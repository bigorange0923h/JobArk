# 可选 AI 网关

AI 默认关闭。本地逐行 JD 提取和字面证据匹配无需外部服务，不能当成语义模型评估或招聘概率。

用户在“AI 模型配置”页面添加 OpenAI 兼容服务商、模型与 API Key，并设置唯一默认模型。API Key 以应用层可逆加密的密文保存在 PostgreSQL，页面和读取 API 仅显示掩码。`JOBARK_AI_CREDENTIAL_ENCRYPTION_KEY` 是数据库外的根密钥；仅 `LOCAL` 环境允许开发默认值，测试与生产必须显式提供。服务商基地址必须为 HTTPS 或本地回环 HTTP，拒绝用户名、密码、查询参数、片段和重定向；本项目不绑定商业 SDK 或代填凭据。

固定任务协议通过**唯一默认启用模型**发送 OpenAI 兼容的结构化输出请求：`POST {base_url}/chat/completions`，请求体包含 `model`、一个系统消息、一个 JSON 用户消息与 `response_format: {"type": "json_object"}`。无默认启用模型、连接失败或结果不符合 Schema 时，均安全失败；每次请求有超时、拒绝重定向且不自动重试，响应体上限 1 MB。领域服务在等待网络前先结束数据库事务，因此外部调用期间不持有事务。

`JOBARK_AI_GATEWAY_URL` 与 `JOBARK_AI_GATEWAY_TOKEN` 已废弃：字段仅为不破坏既有 `backend/.env` 而保留，代码不再读取，**永远不得优先于数据库中的默认模型配置**，并将在后续版本移除；请在「AI 模型配置」页面维护服务商、模型与凭据。

`resume_select_existing_items` 输入包含目标方向和已有简历经历、项目、技能、学历、语言条目；不发送 basics/contact。输出为这五类条目的索引列表，只允许筛选和重排。正文从本地原文复制，模型不能创建新技能或业绩。条目自由文本可能包含个人信息，界面需明确确认；请根据网关部署方的数据留存政策决定是否启用。失败保留基线，未确认候选不会覆盖正式版本。

未配置默认启用模型时 AI 功能返回 409 并提示前往「AI 模型配置」；上游不可用时返回安全文案与 `request_id`，原始资料保持不变。连接测试只发送固定 ping 与 `max_tokens=1`，不携带任何业务数据。本地自动测试替换网关验证成功、错误、越界与确认流程，不代表真实模型服务已联网验收。

`extract_profile_from_resume` 输入为用户明确同意发送的 `resume_text`（可能包含姓名、联系方式、工作与教育信息）及提取规则，不发送 PDF/HTML 原文件。输出遵循请求中的 JSON Schema，包含基本信息、技能、工作和教育候选及原文摘录。后端逐字核对摘录与关键字段，前端要求用户逐项核对并确认；预览不写数据库。若网关未配置，页面返回 409 并保留手工建档路径。此任务当前只有替身测试验证，接入真实网关前需用户确认部署方、数据流向与留存政策。

`jd_requirements_with_verbatim_quotes` 输入仅为所选快照的 `raw_jd`。输出需满足请求中的 JSON Schema；每项 `text` 与 `source_quote` 必须一致，且逐字出现在原文中。标签和强制性仍需人工核对。`POST /api/v1/job-snapshots/{id}/parses` 创建独立的 PARSED 或 FAILED 解析记录；201 表示记录创建成功，不代表解析成功，调用方必须检查 `status`。失败仅保存安全错误码，不持久化上游原始响应，也不改变 JD 快照。GET 同路径读取历史。

匹配接口 `POST /api/v1/matches` 默认不调用网关，只使用快照原文和固定资料修订做本地技能字面证据检索；教育、年限等无法证明的条件保留为未知。它不会自动采用 AI 解析结果，也不生成隐含的满足结论。
