...
def test_jd_failure_keeps_original(db_client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def fake(task: str, input_data: dict[str, Any], schema: dict[str, Any]) -> dict[str, Any]:
        """模拟结构有效但引用不存在的模型响应。"""
        return {
...
def test_optimizer_rejects_invented_indices(db_client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def valid(task: str, input_data: dict[str, Any], schema: dict[str, Any]) -> dict[str, Any]:
        """返回有效选择，不增加或修改条目。"""
