...
领域覆盖：Profile（单例约束、乐观锁、标记已验证必须挂证据、被资料修订引用的事实不可删除、修订快照不含联系方式）；Resume（版本不可变且不存在修改入口、确认候选稿才生成版本且同一候选稿不可重复确认、版本号过期时不留下已生效版本、归档方向禁止新增版本与候选稿、文档模块配置与结构版本非法的拦截）。
...
> 已实现的接口：系统级 `/health`，以及 `/api/v1` 下的 Profile（26 个：档案根、证据、6 类事实、偏好、修订）与 Resume（12 个：简历方向、不可变版本、候选稿确认/丢弃）。尚未实现：Job、Matching、Application、Dashboard。CI 与前端 lint 仍按决策推迟。
