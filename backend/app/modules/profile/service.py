...
def _table_of(entity: EditableEntity) -> Table:
    """取实体对应的表对象。
...
async def _apply_versioned_update(
    ...
    注意:
        比较与写入合并为一条 `UPDATE ... WHERE id = ? AND version = ?`：先查后写存在竞态窗口，
        条件更新由数据库保证并发下只有一个写入成功。写入后刷新实体，使返回值反映增量后的版本号。
    """
    ...
    statement = (
        table.update()
        .where(table.c["id"] == entity.id, table.c["version"] == submitted_version)
...
def _collect_updates(payload: BaseModel) -> dict[str, Any]:
    """从局部更新请求体中取出待写入字段。
...
async def update_profile(session: AsyncSession, payload: ProfileUpdate) -> PersonalProfile:
    ...
    updates = _collect_updates(payload)
    if "links" in updates:
        updates["links"] = [ProfileLink.model_validate(item).model_dump() for item in updates["links"]]
    await _apply_versioned_update(session, profile, payload.version, updates)
    await session.commit()
    return await _reload_profile(session)
...
async def update_evidence(session: AsyncSession, evidence_id: uuid.UUID, payload: EvidenceUpdate) -> ProfileEvidence:
    ...
    await _apply_versioned_update(session, evidence, payload.version, _collect_updates(payload))
    await session.commit()
    return evidence
...
async def update_skill(session: AsyncSession, skill_id: uuid.UUID, payload: SkillUpdate) -> ProfileSkill:
    ...
    await _apply_versioned_update(session, skill, payload.version, updates)
    await session.commit()
    return skill
...
async def update_experience(
    ...
    await _apply_versioned_update(session, experience, payload.version, updates)
    await session.commit()
    return experience
...
async def update_project(session: AsyncSession, project_id: uuid.UUID, payload: ProjectUpdate) -> ProfileProject:
    ...
    await _apply_versioned_update(session, project, payload.version, updates)
    await session.commit()
    return project
...
async def update_education(
    ...
    await _apply_versioned_update(session, education, payload.version, updates)
    await session.commit()
    return education
...
async def update_language(
    ...
    await _apply_versioned_update(session, language, payload.version, updates)
    await session.commit()
    return language
...
async def upsert_preference(session: AsyncSession, payload: PreferenceUpsert) -> ProfilePreference:
    ...
    await _apply_versioned_update(session, existing, payload.version, values)
    await session.commit()
    return existing
